...
def visualize_evaluation(model: torch.nn.Module, eval_data_path: str, 
    ...
    """
    Visualize Burgers equation model evaluation with smooth heatmap comparison plots.
    ...
    # Get ground truth using the numerical solver
    from solvers.burgers1d_solver import solve_burgers1d_spectral
    x_gt, t_gt, h_gt = solve_burgers1d_spectral(
...
def visualize_ncc_dataset(ncc_data: Dict, dataset_dir: Path, config: Dict, prefix: str = ""):
    """
    Visualize NCC dataset distribution for Burgers equation (1D output).
...
def visualize_ncc_classification(
    ...
    """
    Visualize NCC classification in output space (1D for Burgers equation).
...
def visualize_ncc_classification_accuracy_changes(
    ...
    print(f"  Skipping accuracy changes (1D) for burgers1d - similar to heatmap view")
