...
def generate_dataset(
    ...
    Returns:
        Dictionary with keys:
            ...
            "u_gt": (N, 1) ground truth solution (real-valued)
    ...
    u_gt = torch.zeros(N, 1, device=device, dtype=torch.float32)
    u_gt[:, 0] = torch.from_numpy(h_interp.astype(np.float32)).to(device)
