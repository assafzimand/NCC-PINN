"""
Physics-Informed Loss Function for the Convection-Diffusion Equation.

Implements the three-component loss:
    L = w_res*MSE_f + w_ic*MSE_0 + w_bc*MSE_b

where:
- MSE_f: PDE residual loss (h_t + beta*h_x - epsilon*h_xx = 0)
...
def pde_residual(
    ...
    """
    Compute the PDE residual: h_t + beta*h_x - epsilon*h_xx.
    
    For the convection-diffusion equation: h_t + beta*h_x - epsilon*h_xx = 0
    ...
    Args:
        ...
        beta: Convection coefficient (default 1.0)
        epsilon: Diffusion coefficient (default 0.01)
        
    Returns:
        Residual tensor
    """
    residual = h_t + beta * h_x - epsilon * h_xx
    return residual
...
def compute_analytical_indicator_derivatives(
    ...
    For ConvDiff (spatial_dim=1): dpsi/dx, dpsi/dt, d2psi/dx2.
...
def build_loss(**cfg) -> Callable:
    """
    Build physics-informed loss function for the Convection-Diffusion equation.
    
    Args:
        **cfg: Configuration dictionary containing:
            - problem: problem name (e.g., 'conv_diff')
            - conv_diff: dict with 'loss_weights' (residual, ic, bc), 'beta', and 'epsilon'
    ...
    def loss_fn(model: nn.Module, batch: Dict[str, torch.Tensor],
        ...
        """
        Compute physics-informed loss for the Convection-Diffusion equation.
